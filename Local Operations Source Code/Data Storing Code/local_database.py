...
def create_database():
    ...
    BLOB, Real_power BLOB, Apparent_power BLOB, Reactive_power BLOB, Power_factor BLOB)''')
...
def insert_data():
    ...
    sql = 'insert into Power_Usage(Time_sampled, RMS_voltage, RMS_current, Real_power, ' \
          'Apparent_power, Reactive_power, Power_factor)''values(?,?,?,?,?,?,?)'
    data = (time_min, Power_Calc()[0], Power_Calc()[1], Power_Calc()[2], Power_Calc()[3], Power_Calc()[4], Power_Calc()[5],Power_Calc()[6])
...
def create_json():
    ...
    for row in result:
        items.append({'Time_sampled': row[0], 'RMS_voltage': row[1], 'RMS_current': row[2], 'Real_power': row[3],
                      'Apparent_power': row[4], 'Reactive_power': row[5], 'Power_factor': row[6]})
